...
var collection = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2')
  .filterDate('2007-12-01', '2008-01-15')
  .filterBounds(aoi)
...
// Fill the Gaps in the Landsat 7 data 

var image_fill = clippedImage.focal_mean(1, 'square', 'pixels', 25)

var final_image = image_fill.blend(clippedImage)
...
Map.addLayer(final_image.clip(aoi), vizParams, 'Landsat 7 Blended Image');

Map.setCenter(36.8219, -1.2921, 10);
// Training data variable
var label = "Class";
var bands = ['SR_B3', 'SR_B2', 'SR_B1', 'SR_B4', 'SR_B5'];
var input = final_image.select(bands);
// Merge separate feature collections for each land cover class into one
//0= water; 1 = Green Urban Infrastructure (GUI); 2 = built-up areas 3= Bareground/shrubs
var training = Water.merge(GUI).merge(Builtup_areas).merge(Bareground_shrubs);
...
var DATE_RANGE = ee.Filter.dayOfYear(350, 365);
var YEAR_RANGE = ee.Filter.calendarRange(2007, 2007,'year');

// Set the study area bounds and display option for map visualization
...
var LC07_bands = ['ST_B6', 'QA_PIXEL']; // Landsat 8 surface temperature (ST) & QA_Pixel bands

// Create a cloud mask function to be applied to the Landsat 8 images

function cloudMask(final_image) {
  var qa = final_image.select('QA_PIXEL');
  var mask = qa.bitwiseAnd(1 << 3)
    .or(qa.bitwiseAnd(1 << 4));
  return final_image.updateMask(mask.not());
}
...
var values_ST = clip_mean_ST.select("ST_B6"); 
...
var propertiesToDisplay = ['ST_B6_max', 'ST_B6_mean', 'ST_B6_min'];
...
var statGUI = area_GUI.reduceRegion ({
  reducer: ee.Reducer.sum(),
  geometry: aoi,
  scale: 30,
  maxPixels: 1e9
...
Export.image.toDrive({
  image: clip_mean_ST.select('ST_B6'),
  description: 'LST_image_14',
  folder: 'GEE Exports',
  fileNamePrefix: 'LST_14',
  fileFormat: 'GEOTIFF',
  scale: 30,
  region: STUDYBOUNDS
});
...
Export.image.toDrive({
  image: classified,
  description: 'classified_image_14',
  folder: 'GEE Exports',
  scale: 30,
  region: aoi,
  crs: 'EPSG:4326',
